...
import Property from '@/models/Property'; // Ensure this matches your model filename
...
export async function GET(request) {
    try {
        ...
        const myProperties = await Property.find({ 'user.id': id }).sort({ createdAt: -1 });
...
export async function PUT(request) {
    try {
        ...
        const myProperties = await Property.findByIdAndUpdate(id, body).sort({ createdAt: -1 });
